...
const unsigned char g_day_per_mon[MONTH_PER_YEAR] = {31, 28, 31, 30, 31, 30,
                                                     31, 31, 30, 31, 30, 31};
...
u8 RTC_Init(void) {
  ...
  if (BKP_ReadBackupRegister(BKP_DR1) !=
      0x5050)  //从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
  {
    BKP_DeInit();  //复位备份区域

    RCC_RTCCLKConfig(
        RCC_RTCCLKSource_HSE_Div128);  //设置RTC时钟(RTCCLK),选择HSE作为RTC时钟
...
u8 RTC_Get(void) {
  ...
  calendar.min = (temp % 3600) / 60 - 30;  //分钟
  calendar.sec = (temp % 3600) % 60;       //秒钟
  calendar.week = RTC_Get_Week(calendar.w_year, calendar.w_month,
...
unsigned char applib_dt_last_day_of_mon(unsigned char month,
                                        unsigned short year) {
  if ((month == 0) || (month > 12)) {
...
void utc_sec_2_mytime(unsigned int utc_sec, _calendar_obj *result,
                      BOOL daylightSaving) {
  int sec, day;
  unsigned short y;
  unsigned char m;
...
unsigned int mytime_2_utc_sec(_calendar_obj *currTime, BOOL daylightSaving) {
  ...
  utc_time = (unsigned int)no_of_days * SEC_PER_DAY +
             (unsigned int)(currTime->hour * SEC_PER_HOUR +
