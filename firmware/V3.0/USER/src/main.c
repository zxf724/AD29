...
void led_light(void) {
  RTC_Get();
  if ((calendar.hour <= 6) && (calendar.hour >= 2)) {
    // led turns off
...
static void funControl(int argc, char *argv[]) {
  ...
  if (ARGV_EQUAL("CHECK_ALL_MOTO")) {  // check all moto
    ...
    uint16_t crc_test;
    uint8_t report_data_new[8] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06};
    i++;
    DBG_LOG("g_array_ML[%d] = %d", i, g_array_ML[i]);
    ...
    DBG_LOG(
        "report_data[14] = 0x%02x\n\
      report_data[15] = 0x%02x",
        report_data[14], report_data[15]);
    report_data[16] = 0x00;
    report_data[17] = FEND;
  ...
  } else if (ARGV_EQUAL("SEND_HARD")) {
  } else if (ARGV_EQUAL("SEND_OUT")) {
  } else if (ARGV_EQUAL("SEND_OUT_FINISH")) {
  }
}
