 ...
 * @param size   16进制数的个数
 */
void HexArray2Str(uint8_t* in, char* out, uint32_t size) {
  uint32_t i = 0;
  uint8_t* p = (uint8_t*)out, tmp = 0;
 ...
 * @return 返回输出的字符个数
 */
int HexStr2Array(char* in,  uint8_t* out, uint32_t size) 
{
  ...
  for (i = 0; i < size; i++) {
    if (isdigit(*in)) h = *in - '0';
    else if (*in >= 'A' && *in <= 'F') h = *in - 'A' + 10;
    else if (*in >= 'a' && *in <= 'f') h = *in - 'a' + 10;
    else
      break;
    in++;
    *out = h;
    ...
    if (isdigit(*in)) l = *in - '0';
    else if (*in >= 'A' && *in <= 'F') l = *in - 'A' + 10;
    else if (*in >= 'a' && *in <= 'f') l = *in - 'a' + 10;
    else
      break;
    in++;
 ...
 * @param size   数组的长度
 */
void HexEndingSwitc(uint8_t* inout, uint32_t size)
{
  uint32_t i;
  uint8_t *pf, *pr, temp;
